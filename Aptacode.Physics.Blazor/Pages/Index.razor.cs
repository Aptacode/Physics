...
namespace Aptacode.Physics.Blazor.Pages
{
    public class IndexBase : ComponentBase
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            var wall1 = componentBuilder.SetPhysics(false).SetPrimitive(Rectangle.Create(0, -999, 100, 1000)).Build();
            var wall2 = componentBuilder.SetPhysics(false).SetPrimitive(Rectangle.Create(99, 0, 1000, 50)).Build();
            var wall3 = componentBuilder.SetPhysics(false).SetPrimitive(Rectangle.Create(0, 49, 100, 1000)).Build();
            var wall4 = componentBuilder.SetPhysics(false).SetPrimitive(Rectangle.Create(-999, 0, 1000, 50)).Build();
            ...
            for (var i = 0; i < 40; i++)
            {
                var radius = _rand.Next(10, 20);
                var primitiveN = componentBuilder.SetMass(radius * 100)
